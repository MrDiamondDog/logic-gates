...
function GenerateTruthTable() {
  ...
  for (let i = 0; i < Math.pow(2, Utils.inputs.length); i++) {
    ...
    for (let j = 0; j < Utils.inputs.length; j++) {
      ...
      if (powered) input.widgets[0].setPowered(true);else input.widgets[0].setPowered(false);
    }
    // update the nodes
...
window.addEventListener('contextmenu', function (e) {
  e.preventDefault();
  Utils.contextMenu = new ContextMenu(ctx, Utils.mouse.x, Utils.mouse.y, [new ContextMenuItem("AND", function () {
